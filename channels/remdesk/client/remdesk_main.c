 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT32 remdesk_virtual_channel_event_data_received(remdeskPlugin* remdesk,
        ...
        LPVOID pData, UINT32 dataLength, UINT32 totalLength, UINT32 dataFlags)
{
	...
	switch (event)
	{
		...
		if ((error = remdesk_virtual_channel_event_data_received(remdesk,
				pData,
				dataLength,
				totalLength,
				dataFlags)))
        ...
        LPVOID pData, UINT32 dataLength)
{
	...
	remdesk->queue = MessageQueue_New();
        ...
        UINT dataLength)
{
	...
	switch (event)
	{
		case CHANNEL_EVENT_CONNECTED:
			if ((error = remdesk_virtual_channel_event_connected(remdesk,
				pData,
				dataLength)))
			{
