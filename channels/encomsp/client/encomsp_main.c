...
int ctl_interactive = 0;
ENCOMSP_PARTICIPANT_CREATED_PDU g_participantPDU;
...
UINT EncomspParticipantCreated(EncomspClientContext* context, ENCOMSP_PARTICIPANT_CREATED_PDU* participantCreated)
{
	g_participantPDU.Flags = participantCreated->Flags;
	g_participantPDU.FriendlyName = participantCreated->FriendlyName;
	g_participantPDU.GroupId = participantCreated->GroupId;
	g_participantPDU.Length = participantCreated->Length;
	g_participantPDU.ParticipantId = participantCreated->ParticipantId;
	g_participantPDU.Type = participantCreated->Type;
	return 0;
}
...
//				ENCOMSP_PARTICIPANT_CREATED_PDU pdu;
//				if (g_participantPDU.Flags != 0x0002 && ctl_interactive <=3)
//				{
//					ctl_interactive++;
//					pdu.Flags = 0x0003;
//					pdu.ParticipantId = g_participantPDU.ParticipantId;
//					if ((error = encomsp_send_change_participant_control_level_pdu(encomsp->context, &pdu)))
//					{
//						WLog_ERR(TAG,
        ...
        LPVOID pData, UINT32 dataLength)
{
	...
	encomsp->queue = MessageQueue_New();
...
BOOL VCAPITYPE VirtualChannelEntryEx(PCHANNEL_ENTRY_POINTS_EX pEntryPoints, PVOID pInitHandle)
{
	    ...
	    (pEntryPointsEx->MagicNumber == FREERDP_CHANNEL_MAGIC_NUMBER))
	{
		...
		context->ParticipantCreated = EncomspParticipantCreated;
