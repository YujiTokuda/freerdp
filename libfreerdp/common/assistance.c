...
char* freerdp_assistance_construct_expert_blob(const char* name, const char* pass)
{
	...
	sprintf_s(ExpertBlob,
		size,
		"%"PRIdz";NAME=%s%"PRIdz";PASS=%s",
		nameLength,
		name,
		passLength,
		pass);
	...
	size_t* pEncryptedSize)
{
	...
	*((UINT32*) pbIn) = (UINT32)cbPassStubW;
	...
	rc4Ctx = winpr_Cipher_New(WINPR_CIPHER_ARC4_128,
		WINPR_ENCRYPT,
		PasswordHash,
		NULL);
...
static BOOL freerdp_assistance_decrypt2(rdpAssistanceFile* file, const char* password)
{
	...
	if (!freerdp_assistance_crypt_derive_key_sha1(PasswordHash,
		sizeof(PasswordHash),
		DerivedKey,
		sizeof(DerivedKey)))
		goto fail;
	...
	aesDec = winpr_Cipher_New(WINPR_CIPHER_AES_128_CBC,
		WINPR_DECRYPT,
		DerivedKey,
	...
	rdpSettings* settings)
{
	...
	{
		if (freerdp_set_param_string(settings,
	...
	{
		settings->TargetNetAddresses = (TARGET_NET_ADDRESS*) malloc(file->MachineCount * sizeof(TARGET_NET_ADDRESS));
		ZeroMemory(settings->TargetNetAddresses, file->MachineCount * sizeof(TARGET_NET_ADDRESS));
		settings->TargetNetPorts = (UINT32*) malloc(file->MachineCount * sizeof(UINT32));
		ZeroMemory(settings->TargetNetPorts, file->MachineCount * sizeof(UINT32));
		...
		{
			settings->TargetNetAddresses[i].Address = _strdup(file->MachineAddresses[i]);
			settings->TargetNetAddresses[i].Length = strlen(settings->TargetNetAddresses[i].Address);

			settings->TargetNetPorts[i] = file->MachinePorts[i];

			if (!settings->TargetNetAddresses[i].Address)
				return FALSE;
		}
	}
...
void freerdp_assistance_print_file(rdpAssistanceFile* file, void * log, DWORD level)
{
	size_t x;
